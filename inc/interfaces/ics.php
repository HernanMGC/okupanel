...
//					echo 'fetching from '.$url.'<br>';
					...
					foreach ($events as $e){
						...
						if (!empty($e['CLASS']) && @$e['CLASS'] !== 'PUBLIC')
							continue;

						if (!empty($e['STATUS']) && @$e['STATUS'] !== 'CONFIRMED')
							continue;

						$nevent = array(
							'origin' => $url,
							'id' => trim(sanitize_text_field(@$e['UID'])),
							'summary' => trim(sanitize_text_field(okupanel_stripslashes(@$e['SUMMARY']))),
							'description' => trim(sanitize_text_field(okupanel_stripslashes(@$e['DESCRIPTION']))),
							'location' => (!okupanel_clean_room(@$e['LOCATION'], $found) || !$found || (mb_strlen(@$e['LOCATION']) > 15 && $room) ? $room : (isset($e['LOCATION']) && !empty($e['LOCATION']) ? trim(sanitize_text_field(stripslashes($e['LOCATION']))) : null)),
							'status' => isset($e['STATUS']) && !empty($e['STATUS']) ? strtolower(sanitize_text_field($e['STATUS'])) : 'confirmed',
							'created' => okupanel_parse_time(sanitize_text_field(@$e['DTSTAMP'])),
							'created_gmt' => okupanel_parse_time(sanitize_text_field(@$e['DTSTAMP']), true),
							'start' => okupanel_parse_time(sanitize_text_field(@$e['DTSTART'])),
							'start_gmt' => okupanel_parse_time(sanitize_text_field(@$e['DTSTART']), true),
							'end' => okupanel_parse_time(sanitize_text_field(@$e['DTEND'])),
							'end_gmt' => okupanel_parse_time(sanitize_text_field(@$e['DTEND']), true),
							'updated' => okupanel_parse_time(sanitize_text_field(@$e['LAST-MODIFIED'])),
							'updated_gmt' => okupanel_parse_time(sanitize_text_field(@$e['LAST-MODIFIED']), true),
							'htmlLink' => isset($e['URL']) && !empty($e['URL']) ? trim(sanitize_text_field($e['URL'])) : null,//preg_replace('#^(.*?)(\.ics)$#i', '$1.html', $url).'?view=month&action=view&invId='.trim(sanitize_text_field(@$e['UID'])).'&pstat=AC&useInstance=1',//&instStartTime=1504706400000&instDuration=7200000
							'recurrence' => sanitize_text_field(@$e['RRULE']),
							'exceptions' => array(),
						);
