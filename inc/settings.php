				...
				<div>
					...
					<span><?php
						...
						if (okupanel_acf_available()) {
							if (array_key_exists('htmlLink', $e)) {
								$tmp_post_id = url_to_postid( $e['htmlLink']);

								if ($tmp_post_id) {
									$tmp_post_type = get_post_type($tmp_post_id); 
									
									if ($tmp_post_type == 'event') {
										$EM_Event = em_get_event($tmp_post_id, 'post_id');
										$EM_Location = $EM_Event->get_location();

										$okupanel_event_location_is_digital = get_field('okupanel_event_location_is_digital', $EM_Location->post_id);
										if ($okupanel_event_location_is_digital) {
											$okupanel_event_location_url = get_field('okupanel_event_location_url', $EM_Location->post_id);

											echo '<a href="'.$okupanel_event_location_url.'" target="_blank">'.$EM_Location->post_title.'</a>';
